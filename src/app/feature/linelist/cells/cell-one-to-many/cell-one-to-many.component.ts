...
export class CellOneToManyComponent {
  ...
  private async load(id: unknown, tableName: string, foreignKey: string, property: string): Promise<void> {
    this.loading.set(true);
    const records = await this.dataRepository.getByForeignKey(tableName, foreignKey, id);
    this.displayValues.set(
      records.map(r => String((r as Record<string, unknown>)[property] ?? '—')),
    );
